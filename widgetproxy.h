/* $Id: widgetproxy.h,v 1.29 2010/06/29 18:04:37 bergo Exp $ */
...
class WidgetProxy {
 public:
  ...
  void setIcon(const char **xpmdata,const char *text);
  void restorePosition(WindowGeometry *wg);

  GtkWidget * PixButton(const char **xpmicon, const char *caption);
...
class ModalDialog : public WidgetProxy {
 public:
  ...
  void setDismiss(GtkObject *obj,const char *sig);
...
class NonModalDialog : public ModalDialog {
 public:
  NonModalDialog(const char *title);
  ~NonModalDialog();
  virtual void show();
...
class LayoutBox {
 public:
  ...
  bool drawBoxedString(int x,int y,int w,int h,int f,const char *s,const char *sep);
  bool prepared();
