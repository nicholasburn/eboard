/* $Id: position.cc,v 1.63 2010/06/29 18:04:37 bergo Exp $ */
...
void Position::setLastMove(const char *s) {
  LastMove=s;
}
...
void Position::moveAnyNotation(const char *m,piece color,variant Vr) {
  int i,x,y,ml;
  piece pr;
  char xlate[12];
...
void Position::moveStdNotation(const char *m,piece color,variant Vr) {
  int from[2],to[2];
  int istake, isdrop;
  piece what, prom;
  const char *p;
