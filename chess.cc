...
void ChessGame::updateClockAndInfo2(int wclockms, int bclockms,
				    int blacktomove, const char *infoline,
				    bool sndflag)
{
  if (myboard) {
    myboard->freeze();
...
void ChessGame::updatePosition2(Position &p,int movenum,int blacktomove,
			       int wclockms,int bclockms,const char *infoline, 
			       bool sndflag) {
...
void ChessGame::endGame(const char *reason,GameResult _result) {
  if (over)
...
bool ChessGame::ParsePgnGame(zifstream &f, 
  ...
  */
  ...
  static const char *token  =     "1234567890*/KQBNRP@kqbnrp=+#abcdefgh-Ox$";
  static const char *digits =     "1234567890";
  static const char *resultset =  "01*";
...
PGNpair::PGNpair(const char *n, const char *v) {
  name=n;
  value=v;
}
...
void PGNheader::set(const char *n,const char *v) {
...
void PGNheader::setIfAbsent(const char *n,const char *v) {
  vector<PGNpair *>::iterator li;
  for(li=header.begin();li!=header.end();li++)
    if ( (*li)->name == n )
      return;
