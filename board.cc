/* $Id: board.cc,v 1.125 2010/06/29 18:04:37 bergo Exp $ */
...
void RootBoard::clockString(int val,char *z,int maxlen) {
  ...
  const int mrdiv[4] = { 1, 100, 10, 1 };
  char fmt[32];
  int cv,mr,neg,H,M,S,TS, ed;
  cv=val;
  if (cv<0) {
    cv=-cv;
    neg=1;
  ...
  TS = H*3600 + M*60 + S;
  ed = global.MsecDigits;
  if (ed > 3) ed = 3; if (ed<1) mr=0;
  if (ed < 0) ed = 0;
  mr /= mrdiv[ed];

  if (TS >= global.MsecThreshold || mr==0) {
    if (H!=0) {
    ...
    if (H==0) {
      snprintf(fmt,32,"%%s%%.2d:%%.2d.%%.%dd", ed);
      snprintf(z,maxlen,fmt,neg?minus:&minus[1],M,S,mr);
    } else {
      snprintf(fmt,32,"%%s%%d:%%.2d:%%.2d.%%.%dd", ed);
      snprintf(z,maxlen,fmt,neg?minus:&minus[1],H,M,S,mr);
    }
  }
}
...
void Board::setInfo(int idx,const char *s) {
...
void Board::update(bool sndflag) {
  ...
  if ((global.HilightLastMove)||(global.AnimateMoves)) {
	...
	if ((global.AnimateMoves)&&(gdk_window_is_viewable(yidget->window))) {
	  ...
	  rate = global.SmootherAnimation ? 8 : 4;
	  for(i=0;i<LastMove.size();i++) {
	    if (ef)
	  ...
	  if ((!animes.empty())&&(!gotAnimationLoop)) {
	    ...
	    if (rate==4)
	      tag=gtk_timeout_add(30,board_animate,this);
	    else
	      tag=gtk_timeout_add(15,board_animate,this);
