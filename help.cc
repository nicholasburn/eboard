...
namespace Help {
  ...
  KeysDialog::KeysDialog() : ModalDialog(N_("Help: Keys")) {
    ...
    // changed this code to allow new commands to be added without losing
    // the translations of the existing ones (1.1.2) -- Felipe
    t.push_back(_("Anywhere:"));

    s[0].push_back(_("F3: Go to previous pane."));
    s[0].push_back(_("F4: Go to next pane."));
    s[0].push_back(_("F5: Go to the main board pane."));
    s[0].push_back(_("F6: Go to the console pane."));
    s[0].push_back(_("F7: Go to the seek graph pane (if available)."));
    s[0].push_back(_("F8: Toggle shortcut bar visibility."));
    s[0].push_back(_("Page Up/Page Down: Scroll the text console."));
    s[0].push_back(_("Ctrl+Left: Backward 1 halfmove."));
    s[0].push_back(_("Ctrl+Right: Forward 1 halfmove."));
    s[0].push_back(_("Ctrl+F: Find upwards (main console buffer)."));
    s[0].push_back(_("Ctrl+G: Find previous."));
    s[0].push_back(_("Ctrl+T: Toggle timestamps."));

    t.push_back(_("Input Box:"));

    s[1].push_back(_("Up/Down: move thru input history."));
    s[1].push_back(_("Enter: send text line."));
    s[1].push_back(_("ESC: switch chat/command mode."));

    t.push_back(_("Syntaxisms:"));

    s[2].push_back(_(".. entering %prefix text will set a chat prefix."));
    s[2].push_back(_(".. entering %do scriptname runs scriptname."));

    z[0] = 0;
    for(i=0;i<3;i++) {
      strcat(z, t[i].c_str());
      strcat(z, "\n");
      for(lsi=s[i].begin();lsi!=s[i].end();lsi++) {
	strcat(z, (*lsi).c_str());
	strcat(z, "\n");
      }
      strcat(z, "\n");
    ...
    gtk_widget_grab_default(ok);
    gtk_window_set_default_size(GTK_WINDOW(widget),450, 500);
  ...
  char * DebugDialog::grabOutput(const char *cmdline) {
    FILE *f;
    bool ok = false;
    char *c;
    f=popen(cmdline,"r");
