/* $Id: util.cc,v 1.37 2010/06/29 18:04:37 bergo Exp $ */
...
void FileFinder::addDirectory(const char *dir) { path.push_back(string(dir)); }
void FileFinder::addDirectory(string &dir) { path.push_back(string(dir)); }

void FileFinder::addMyDirectory(const char *dir) {
  string s;
...
int FileFinder::find(const char *name,char *fullpath) {
  int i,j;
  char fullname[512];
...
ExactStringPat::ExactStringPat(const char *pat) {
  int i,j;
  j=strlen(pat);
  for(i=0;i<j;i++)
...
void SetPat::addToSet(const char *s) {
  while(*s)
    addToSet(*(s++));
}
...
char * PatternMatcher::getToken(int index) {
  ...
  for(i=0,trav=matches.begin();i<index;i++) {
    if (trav==matches.end()) return((char *) "<null>");
    trav++;
  }
...
void ExtPatternMatcher::set(const char *hrp) {
  int i,L,tc;
