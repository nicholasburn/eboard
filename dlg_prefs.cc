/* $Id: dlg_prefs.cc,v 1.57 2010/06/30 14:23:54 bergo Exp $ */
...
const char * PreferencesDialog::FontSample[NFONTS] = {
  "52:38",
  "White Capablanca",
  "38. N@c5+",  
...
PreferencesDialog::PreferencesDialog() : ModalDialog(N_("Preferences")) {
  ...
  GtkWidget *icv, *icl, *ict1, *icl1, *ifr1, *ifr2, *ifv1, *ifv2, 
    *icl2, *icl3, *icl4, *icl5, *icl6, *spf, *sph;
  GSList *spl;

  // -- sound
  static const char *zcaption[]={N_("Opponent Moved"),
				 N_("Draw Offered"),
				 N_("Private Tell"),
				 N_("ICS Challenge"),
				 N_("Time Running Out"),
  ...
  */
  ...
  icl4=gtk_label_new(_("Show milliseconds when below: "));
  icl6=gtk_label_new(_("(HH:MM:SS)"));
  msecth=gtk_entry_new();
  {
    int hh,mm,ss;
    hh = global.MsecThreshold / 3600;
    mm = (global.MsecThreshold % 3600) / 60;
    ss = (global.MsecThreshold % 3600) % 60;
    snprintf(z,64,"%.2d:%.2d:%.2d",hh,mm,ss);
    gtk_entry_set_text(GTK_ENTRY(msecth),z);
  }

  icl5=gtk_label_new(_("Subsecond precision digits: "));
  msecdig = gtk_entry_new();
  snprintf(z,64,"%d",global.MsecDigits);
  gtk_entry_set_text(GTK_ENTRY(msecdig),z);
  ...
  gtk_table_attach_defaults(GTK_TABLE(ict1), icl4, 0,1, 2,3);
  gtk_table_attach_defaults(GTK_TABLE(ict1), msecth, 1,2, 2,3);
  gtk_table_attach_defaults(GTK_TABLE(ict1), icl6, 2,3, 2,3);

  gtk_table_attach_defaults(GTK_TABLE(ict1), icl5, 0,1, 3,4);
  gtk_table_attach_defaults(GTK_TABLE(ict1), msecdig, 1,2, 3,4);
  ...
  Gtk::show(icl1, sbacke, icl2, lowtime, icl3, ict1,
	    ...
	    sgb, chsb, coct, wget, hsb, icv, spf, sph, icl4, msecth, icl5, 
	    msecdig, icl6, NULL);
...
void PreferencesDialog::Apply() {
  ...
  */
  ...
  p=gtk_entry_get_text(GTK_ENTRY(msecth));
  nval=TimeControl::stringToSeconds(p);
  if (nval!=global.MsecThreshold) {
    changed=1;
    global.MsecThreshold=nval;
  }

  p=gtk_entry_get_text(GTK_ENTRY(msecdig));
  nval=atoi(p);
  if (nval!=global.MsecDigits) {
    changed=1;
    global.MsecDigits=nval;
  }
...
void prefs_cfont(GtkWidget *w,gpointer data) {
  ...
  if (me->FontBeingEdited < 0) {
    cerr << "**** eboard internal error: you should never see this, report it\n";
    return;
  }
