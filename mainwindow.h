/* $Id: mainwindow.h,v 1.75 2010/06/29 18:04:37 bergo Exp $ */
		   ...
		   public IONotificationInterface
{
 public:
  ...
  void openServer(const char *host,int port,Protocol *protocol,
		  const char *helper);
 ...
 private:
  ...
  void historyUp();
  void historyDown();
  void tryConnect(const char *host,int port,Protocol *protocol,
		  const char *helper);
