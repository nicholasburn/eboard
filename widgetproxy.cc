/* $Id: widgetproxy.cc,v 1.39 2010/06/29 18:04:37 bergo Exp $ */
...
void WidgetProxy::setIcon(const char **xpmdata,const char *text) {
  GdkPixmap *icon;
  GdkBitmap *mask;
  GtkStyle *style;
...
GtkWidget * WidgetProxy::PixButton(const char **xpmicon, const char *caption) {
  GdkPixmap *d;
  GdkBitmap *mask;
  GtkWidget *b, *p, *v, *t;
  GtkStyle *style;
...
ModalDialog::ModalDialog(const char *title) {
...
void ModalDialog::setDismiss(GtkObject *obj,const char *sig) {
...
NonModalDialog::NonModalDialog(const char *title) : ModalDialog(title) {
...
bool LayoutBox::drawBoxedString(int x,int y,int w,int h,
				int f,const char *s,const char *sep) 
{
  int cy, lskip;
