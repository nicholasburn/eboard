/* $Id: util.h,v 1.21 2010/06/29 18:04:37 bergo Exp $ */
...
class FileFinder {
 public:
  ...
  void addMyDirectory(const char *dir);
  void addMyDirectory(string &dir);

  int  find(const char *name,char *fullpath);
...
class CIExactStringPat : public ExactStringPat {
 public:
  CIExactStringPat(const char *pat) : ExactStringPat(pat) { }
...
class ExtPatternMatcher : public PatternMatcher {
 public:
  ...
  void set(const char *hrp); // set from a human readable pattern
