/* $Id: network.cc,v 1.52 2010/06/29 18:04:37 bergo Exp $ */
...
DirectConnection::DirectConnection(const char *hostname,int port) {
...
void DirectConnection::writeLine(const char *obuffer) {
  if (Connected) {
    const char *tmp;
...
void IncomingConnection::writeLine(const char *obuffer) {
  if (Connected) {
    const char *tmp;
    tmp = global.filter(obuffer);
    write(netsocket,tmp,strlen(tmp));
...
PipeConnection::PipeConnection(const char *helperbin,const char *arg1,
			       const char *arg2,
			       const char *arg3, 
			       const char *arg4) {
  init();
...
void PipeConnection::setHandshake(const char *s) {
  handshake=s;
}

PipeConnection::PipeConnection(const char *host,int port, 
			       const char *helperbin,const char *helpersuffix)
{
  char z[256];
...
void PipeConnection::writeLine(const char *obuffer) {
  int ew,rem,ec,tol=0;
  fd_set wfd;
  struct timeval tv;
...
void FallBackConnection::writeLine(const char *tbuffer) {
  if (Connected)
