/* $Id: text.cc,v 1.43 2010/06/29 18:04:37 bergo Exp $ */
...
void Text::append(const char *msg,int color,Importance imp) {
...
void Text::append(const char *msg,const char *msg2,int color, Importance imp) {
  char *d;
...
void TextSet::append(const char *msg,int color,Importance imp) {
  list<Text *>::iterator ti;
  for(ti=targets.begin();ti!=targets.end();ti++)
    (*ti)->append(msg,color,imp);
}

void TextSet::append(const char *msg,const char *msg2,int color,Importance imp) {
  list<Text *>::iterator ti;
  for(ti=targets.begin();ti!=targets.end();ti++)
...
void DetachedConsole::setFilter(const char *s) {
  inner->Filter.set(s);
  updateFilterLabel();
}
...
bool TextFilter::accept(const char *textline) {
  int i,j;
